...
| `NavUp` / `NavDown` / `NavSelect` / `NavBack` | `w` / `a` / `s` / `d` | HTML nav keys. Valid: `w a s d`, `e`/`use`, `shift`/`speed`, `ctrl`/`duck`, `space`/`jump`, `r`/`reload`, `mouse1`, `mouse2`, `tab`, `f`/`inspect`, or `none` to disable. Unknown names keep the default. |
